...
(defparameter *js*
  (ps
    ...
    (defun init-game (state canvas &optional (width 800) (height 300))
    ...
    (defparameter *slides*
      (let ((x-def (+ "Let \\(\\mathbf{x}_j(t) \\in \\mathbb{R}^2 \\) "
		      "be the position of the "
		      "\\(j^{\\text{th}}\\) "
		      "prey at time \\(t\\)."))
	    (z-def (+ "Let \\(\\mathbf{z}(t) \\in \\mathbb{R}^2 \\) "
		      "be the position of the "
		      "predator at time \\(t\\)."))
	    (newton (+ "Assume the prey and predator "
		       "obey Newton's second law: "
		       "\\(\\mathbf{F} = m \\mathbf{a}\\), "
		       "and that the prey and predator "
		       "have friction with the ground."))
	    (prey-model2 (+ "\\(m \\frac{d^2 \\mathbf{x}_j}{dt^2} "
			    "= \\mathbf{F}_{\\text{prey$\\to$prey}_j} "
			    "+ \\mathbf{F}_{\\text{predator$\\to$prey}_j} "
			    "- \\mu \\frac{d \\mathbf{x}_j}{\dt} \\),"))
	    (pred-model2 (+ "\\(m \\frac{d^2 \\mathbf{z}}{dt^2} "
			    "= \\mathbf{F}_{\\text{prey$\\to$predator}} "
			    "- \\mu \\frac{d \\mathbf{z}}{\dt} \\)."))
	    (prey-model1 (+ "\\(\\frac{d \\mathbf{x}_j}{dt} "
			    "= \\mathbf{F}_{\\text{prey$\\to$prey}_j} "
			    "+ \\mathbf{F}_{\\text{predator$\\to$prey}_j}\\),"))
	    (pred-model1 (+ "\\(\\frac{d \\mathbf{z}}{dt} "
			    " = \\mathbf{F}_{\\text{prey$\\to$predator}}\\)."))
	    (prey-model-explicit
	      (+ "\\(\\frac{d \\mathbf{x}_j}{dt} "
		 "= \\frac{1}{N} \\sum_{k=1,k \\neq j}^{N} "
		 "\\left(\\frac{\\mathbf{x}_j - "
		 "\\mathbf{x}_k}{|\\mathbf{x}_j - \\mathbf{x}_k|^2} "
		 "- a (\\mathbf{x}_j - \\mathbf{x}_k) \\right) "
		 " + b \\frac{\\mathbf{x}_j - \\mathbf{z}}"
		 "{|\\mathbf{x}_j - \\mathbf{z}|^2} \\),"))
	    (pred-model-explicit
	      (+ "\\(\\frac{d \\mathbf{z}}{dt} "
		 " = \\frac{c}{N} "
		 "\\sum_{k=1}^{N} \\frac{\\mathbf{x}_k - \\mathbf{z}}"
		 "{|\\mathbf{x_k} - \\mathbf{z}|^p} \\).")))
	(list
	 ...
	 (make-slide "The model" (list x-def z-def newton))
	 (make-slide "The model" (list x-def z-def newton prey-model2))
	 (make-slide "The model" (list x-def z-def newton prey-model2 pred-model2))
	 (make-slide "The model" (list x-def
				       z-def
				       newton
				       prey-model2
				       pred-model2
